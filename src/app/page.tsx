...
// --- CHAT BUBBLES ---
const MessageBubble = memo(({ m, isTyping, isDark }: { m: Message; isTyping?: boolean; isDark: boolean }) => {
  if (m.role === 'user') {
    return (
      <div className="flex justify-end mb-8">
        <div className={`${isDark ? 'bg-[#333537] text-slate-200' : 'bg-[#f0f4f9] text-slate-800'} px-5 py-3 rounded-[24px] rounded-br-sm max-w-[85%] md:max-w-[70%] text-[15px] leading-relaxed shadow-sm`}>
  ...
  return (
    <div className="flex justify-start gap-4 mb-8">
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-lg shrink-0 mt-1 shadow-sm border ${isDark ? 'bg-amber-900/30 border-amber-800/50' : 'bg-amber-100 border-amber-200'}`}>
        🏗️
      </div>
      <div className={`flex-1 max-w-[90%] md:max-w-[80%] text-[15px] leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-800'} ${isTyping ? 'typing-cursor' : ''}`}>
        <ReactMarkdown
          remarkPlugins={[remarkMath, remarkGfm]}
          rehypePlugins={[rehypeKatex]}
          components={{
            h1: ({node, ...props}) => <h1 className={`text-2xl font-semibold mt-6 mb-4 ${isDark ? 'text-white' : 'text-slate-900'}`} {...props} />,
            h2: ({node, ...props}) => <h2 className={`text-xl font-semibold mt-5 mb-3 ${isDark ? 'text-white' : 'text-slate-900'}`} {...props} />,
            h3: ({node, ...props}) => <h3 className={`text-lg font-medium mt-4 mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`} {...props} />,
            p: ({node, ...props}) => <p className="mb-4" {...props} />,
            ul: ({node, ...props}) => <ul className={`list-disc pl-5 mb-4 space-y-1.5 ${isDark ? 'marker:text-slate-600' : 'marker:text-slate-400'}`} {...props} />,
            ol: ({node, ...props}) => <ol className={`list-decimal pl-5 mb-4 space-y-1.5 ${isDark ? 'marker:text-slate-600' : 'marker:text-slate-400'}`} {...props} />,
            strong: ({node, ...props}) => <strong className={`font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`} {...props} />,
            table: ({node, ...props}) => (
              <div className={`overflow-x-auto my-6 rounded-2xl border ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
                <table className="w-full text-left border-collapse" {...props} />
              </div>
            ),
            thead: ({node, ...props}) => <thead className={`${isDark ? 'bg-[#1e1f20] border-slate-700' : 'bg-[#f0f4f9] border-slate-200'} border-b`} {...props} />,
            th: ({node, ...props}) => <th className={`py-2.5 px-4 font-medium whitespace-nowrap ${isDark ? 'text-slate-300' : 'text-slate-700'}`} {...props} />,
            td: ({node, ...props}) => <td className={`py-2.5 px-4 border-b ${isDark ? 'border-slate-700/50' : 'border-slate-100'}`} {...props} />,
          }}
        >
          {m.content}
        </ReactMarkdown>
    ...
    <div className={`flex h-screen overflow-hidden font-sans transition-colors duration-300 ${isDarkMode ? 'bg-[#131314] text-slate-200' : 'bg-white text-slate-800'}`}>
      ...
      <main className="flex-1 flex flex-col relative h-full min-w-0 transition-all duration-300">
        ...
        <div className="h-[64px] shrink-0 w-full flex items-center md:hidden pl-[64px]">
           {!isSidebarOpen && <span className={`text-[18px] font-medium tracking-wide select-none ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>CivilGPT</span>}
        </div>
        ...
        {activeTab === 'chat' && (
          <div className="flex-1 flex flex-col relative overflow-hidden h-full">
              ...
              <div className="max-w-3xl mx-auto w-full">
                ...
                {isFetchingHistory ? (
                  <div className="flex items-center justify-center h-full pt-20">
                    <span className={`animate-spin text-3xl ${isDarkMode ? 'text-slate-700' : 'text-[#e8eaed]'}`}>⏳</span>
                  </div>
                ) : messages.length === 0 ? (
                  <div className="flex flex-col h-full text-left space-y-3 pt-[15vh]">
                    <h2 className={`text-4xl font-medium tracking-tight ${isDarkMode ? 'text-slate-400' : 'text-[#c4c7c5]'}`}>Hello, Engineer.</h2>
                    <p className={`text-2xl font-medium max-w-xl leading-relaxed ${isDarkMode ? 'text-slate-500' : 'text-[#c4c7c5]'}`}>How can I help you with your structural analysis today?</p>
                  </div>
                ) : (
                  messages.map((m, index) => (
                    <MessageBubble key={m.id} m={m} isTyping={isLoading && index === messages.length - 1 && m.role === 'assistant'} isDark={isDarkMode} />
                  ))
                )}
            ...
            <div className={`absolute bottom-0 left-0 w-full pt-6 pb-6 px-4 md:px-8 bg-gradient-to-t ${isDarkMode ? 'from-[#131314] via-[#131314] to-transparent' : 'from-white via-white to-transparent'}`}>
              <div className="max-w-3xl mx-auto w-full">
                
                {uploadStatus && (
                  <div className={`mb-3 text-[12px] px-4 py-2 rounded-full inline-flex font-medium shadow-sm ${uploadStatus.type === 'success' ? (isDarkMode ? 'bg-emerald-900/30 text-emerald-400' : 'bg-[#e6f4ea] text-[#137333]') : (isDarkMode ? 'bg-red-900/30 text-red-400' : 'bg-[#fce8e6] text-[#c5221f]')}`}>
                    {uploadStatus.message}
                  </div>
                )}

                <form onSubmit={customHandleSubmit} className={`relative flex items-end rounded-[32px] p-2 pr-4 transition-all focus-within:shadow-md border ${isDarkMode ? 'bg-[#1e1f20] border-transparent focus-within:bg-[#333537] focus-within:border-slate-600' : 'bg-[#f0f4f9] border-transparent focus-within:bg-white focus-within:border-slate-200'}`}>
                  ...
                  <button 
                    ...
                    className={`p-2.5 rounded-full transition-colors shrink-0 mb-0.5 ml-1 ${isDarkMode ? 'text-slate-400 hover:text-slate-200 hover:bg-slate-700' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-200/50'}`}
                  ...
                  <textarea 
                    className={`flex-1 bg-transparent text-[15px] px-3 py-3 focus:outline-none resize-none min-h-[48px] max-h-[200px] ${isDarkMode ? 'text-slate-200 placeholder-slate-500' : 'text-slate-800 placeholder-slate-500'}`}
                    rows={1} 
                    value={myInput} 
                  ...
                  <button 
                    ...
                    className={`p-2.5 rounded-full shrink-0 mb-0.5 transition-colors ${myInput.trim() === "" || isLoading ? (isDarkMode ? 'text-slate-600' : 'text-slate-300') : (isDarkMode ? 'text-amber-400 hover:bg-slate-700' : 'text-slate-800 hover:bg-slate-200/50')}`}
                  >
                ...
                <div className={`text-center mt-2.5 text-[11px] ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
        ...
        {activeTab === 'calculator' && (
          <div className="flex-1 overflow-y-auto px-4 md:px-8 pt-4 pb-12 h-full">
            <div className="max-w-4xl mx-auto w-full">
              <div className={`border rounded-[32px] p-8 md:p-10 shadow-sm mt-8 md:mt-2 transition-colors ${isDarkMode ? 'bg-[#1e1f20] border-slate-800' : 'bg-white border-slate-200'}`}>
                <div className="mb-8">
                  <h2 className={`text-3xl font-medium tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>Mix Design Calculator</h2>
                  <p className={`mt-2 text-[15px] ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>IS 10262:2019 Absolute Volume Method</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                  ...
                  <div className="space-y-5">
                    <div>
                      <label className={`block text-[13px] font-medium mb-1.5 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>Target Grade (fck)</label>
                      <select value={fck} onChange={e => setFck(Number(e.target.value))} className={`w-full p-3 rounded-[16px] border outline-none text-[14px] transition-colors ${isDarkMode ? 'bg-[#131314] border-slate-700 text-slate-200 focus:border-slate-500' : 'bg-[#f0f4f9] border-slate-200 text-slate-800 focus:bg-white focus:border-slate-300'}`}>
                    ...
                    <div>
                      <label className={`block text-[13px] font-medium mb-1.5 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>Standard Deviation (s) - N/mm²</label>
                      <input type="number" step="0.1" value={stdDev} onChange={e => setStdDev(Number(e.target.value))} className={`w-full p-3 rounded-[16px] border outline-none text-[14px] transition-colors ${isDarkMode ? 'bg-[#131314] border-slate-700 text-slate-200 focus:border-slate-500' : 'bg-[#f0f4f9] border-slate-200 text-slate-800 focus:bg-white focus:border-slate-300'}`} />
                    </div>

                    <div>
                      <label className={`block text-[13px] font-medium mb-1.5 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>Water-Cement Ratio</label>
                      <input type="number" step="0.01" value={wcRatio} onChange={e => setWcRatio(Number(e.target.value))} className={`w-full p-3 rounded-[16px] border outline-none text-[14px] transition-colors ${isDarkMode ? 'bg-[#131314] border-slate-700 text-slate-200 focus:border-slate-500' : 'bg-[#f0f4f9] border-slate-200 text-slate-800 focus:bg-white focus:border-slate-300'}`} />
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className={`block text-[13px] font-medium mb-1.5 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>Sp. Gravity (Cement)</label>
                        <input type="number" step="0.01" value={sgCement} onChange={e => setSgCement(Number(e.target.value))} className={`w-full p-3 rounded-[16px] border outline-none text-[14px] transition-colors ${isDarkMode ? 'bg-[#131314] border-slate-700 text-slate-200 focus:border-slate-500' : 'bg-[#f0f4f9] border-slate-200 text-slate-800 focus:bg-white focus:border-slate-300'}`} />
                      </div>
                      <div>
                        <label className={`block text-[13px] font-medium mb-1.5 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>Max Water (kg/m³)</label>
                        <input type="number" value={waterContent} onChange={e => setWaterContent(Number(e.target.value))} className={`w-full p-3 rounded-[16px] border outline-none text-[14px] transition-colors ${isDarkMode ? 'bg-[#131314] border-slate-700 text-slate-200 focus:border-slate-500' : 'bg-[#f0f4f9] border-slate-200 text-slate-800 focus:bg-white focus:border-slate-300'}`} />
                      </div>
                    </div>

                    <button onClick={calculateMix} className={`w-full py-3.5 mt-2 rounded-[24px] font-medium text-[14px] transition-colors shadow-sm ${isDarkMode ? 'bg-amber-500/20 text-amber-400 hover:bg-amber-500/30' : 'bg-slate-800 text-white hover:bg-slate-700'}`}>
                      Calculate Proportions
                    </button>
                  </div>

                  {/* Results */}
                  <div className={`rounded-[24px] p-8 h-full transition-colors border ${isDarkMode ? 'bg-[#131314] border-slate-800' : 'bg-[#f0f4f9] border-transparent'}`}>
                    <h3 className={`text-[12px] font-medium uppercase tracking-wider mb-5 ${isDarkMode ? 'text-slate-500' : 'text-slate-500'}`}>Output per m³</h3>
                    
                    {mixResult ? (
                      <div className="space-y-4">
                        <div className={`flex justify-between items-end border-b pb-2.5 ${isDarkMode ? 'border-slate-700/50' : 'border-slate-200/60'}`}>
                          <span className={`text-[14px] ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Target Mean Strength ($f_m$)</span>
                          <span className={`text-[16px] font-medium ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>{mixResult.targetStrength} N/mm²</span>
                        </div>
                        <div className={`flex justify-between items-end border-b pb-2.5 ${isDarkMode ? 'border-slate-700/50' : 'border-slate-200/60'}`}>
                          <span className={`text-[14px] ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Cement</span>
                          <span className={`text-[16px] font-medium ${isDarkMode ? 'text-amber-400' : 'text-slate-800'}`}>{mixResult.cement} kg</span>
                        </div>
                        <div className={`flex justify-between items-end border-b pb-2.5 ${isDarkMode ? 'border-slate-700/50' : 'border-slate-200/60'}`}>
                          <span className={`text-[14px] ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Water</span>
                          <span className={`text-[16px] font-medium ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>{mixResult.water} kg</span>
                        </div>
                        <div className={`flex justify-between items-end border-b pb-2.5 ${isDarkMode ? 'border-slate-700/50' : 'border-slate-200/60'}`}>
                          <span className={`text-[14px] ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Fine Aggregate (FA)</span>
                          <span className={`text-[16px] font-medium ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>{mixResult.fa} kg</span>
                        </div>
                        <div className={`flex justify-between items-end border-b pb-2.5 ${isDarkMode ? 'border-slate-700/50' : 'border-slate-200/60'}`}>
                          <span className={`text-[14px] ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Coarse Aggregate (CA)</span>
                          <span className={`text-[16px] font-medium ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>{mixResult.ca} kg</span>
                        </div>

                        <div className="pt-4 mt-4">
                          <p className={`text-[12px] mb-1 font-medium ${isDarkMode ? 'text-slate-500' : 'text-slate-500'}`}>Mix Ratio (C : FA : CA)</p>
                          <p className={`text-[26px] font-medium tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{mixResult.ratio}</p>
                        </div>
                      </div>
                    ) : (
                      <div className={`h-full flex flex-col items-center justify-center text-center space-y-3 pb-12 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                        <p className="text-[14px]">Adjust your parameters and click calculate to view the mix proportions.</p>
                      </div>
                    )}
                  </div>
                </div>
