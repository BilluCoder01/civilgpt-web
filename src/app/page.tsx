                    ...
                    if (href?.startsWith("viewer://")) {
                      ...
                      return (
                        <span className="block mt-2 mb-2">
                          <button 
                            type="button"
                            onClick={() => onOpenViewer(docId, page, "IS Standard Document", citationText)}
                            className={`group inline-flex items-center gap-2.5 px-4 py-2.5 rounded-[16px] text-[13px] font-semibold tracking-wide border transition-all duration-300 hover:shadow-md ${isDark ? 'bg-[#1e1f20] border-slate-700 hover:border-amber-500/50 text-slate-300 hover:text-amber-400' : 'bg-white border-slate-200 hover:border-amber-300 text-slate-700 hover:text-amber-700 shadow-sm'}`}
                          >
                            <div className={`flex items-center justify-center w-6 h-6 rounded-md ${isDark ? 'bg-[#333537] group-hover:bg-amber-500/20 text-slate-400 group-hover:text-amber-400' : 'bg-slate-100 group-hover:bg-amber-50 text-slate-500 group-hover:text-amber-600'}`}>
                               <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5">
                                 <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                               </svg>
                            </div>
                            <span className="truncate max-w-[220px] md:max-w-[400px]">{citationText.replace('Source:', 'View Source:')}</span>
                          </button>
                        </span>
                      );
                    }
